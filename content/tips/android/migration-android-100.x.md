...
Android Studio のビルド ツールは Gradle を使用し、maven リポジトリーとライブラリーの参照を変更します。
...
`Geometry` オブジェクトのコンストラクターを使用すると、既知の座標を使用してジオメトリーを作成できますが、作成後にそのジオメトリーを変更することはできません。

ジオメトリー ビルダー（`GeometryBuilder`）を使用すると、ゼロから新しいジオメトリーを作成したり、既存のジオメトリーを基に、ジオメトリーを変更することができます。
