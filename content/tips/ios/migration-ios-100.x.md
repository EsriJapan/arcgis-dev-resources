...
`AGSGeometry` オブジェクトのコンストラクターを使用すると、既知の座標を使用してジオメトリを作成できますが、作成後にそのジオメトリーを変更することはできません。
ジオメトリー ビルダー（`AGSGeometryBuilder`）を使用すると、ゼロから新しいジオメトリを作成したり、既存のジオメトリを基に、ジオメトリーを変更することができます。
