...
1. `C:\inetpub\wwwroot\arcgis_js_api\library\4.13\init.js` をテキスト エディターで開きます。
1. `[HOSTNAME_AND_PATH_TO_JSAPI]` を検索して `fqdn.tld/arcgis_js_api/library/4.13/` に置き換えます。
1. `C:\inetpub\wwwroot\arcgis_js_api\library\4.13\dojo\dojo.js` をテキスト エディターで開きます。
1. `[HOSTNAME_AND_PATH_TO_JSAPI]` を検索して `fqdn.tld/arcgis_js_api/library/4.13/` に置き換えます。
...
```html
<script src="https://www.example.com/arcgis_js_api/library/4.13/init.js"></script>
```
...
<html>
  <head>
    ...
    <link rel="stylesheet" href="https://www.example.com/arcgis_js_api/library/4.13/esri/css/main.css" />
    <style>
