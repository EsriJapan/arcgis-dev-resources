...
このチュートリアルで使用するセキュアな ArcGIS ロケーション サービスにアクセスするには、ArcGIS Location Platform または ArcGIS Online アカウントを使用して、API キー認証またはユーザ認証を実装する必要があります。

このチュートリアルでは、APIキー認証またはユーザー認証を実装することができます。以下の違いを比較してください。

#### API キー認証
* ユーザーはサインインする必要がありません。
* 適切な権限を持つ API キーのクレデンシャルを作成する必要があります。
* API キーは長寿命のアクセストークンです。
* サービス使用量は API キーの所有者/開発者に請求されます。
* 実装が最も簡単な認証方法です。
...
このチュートリアルで使用するセキュアなリソースにアクセスする権限を持つ、新しい API キーのアクセストークンを作成します。
...
このチュートリアルで使用するセキュアなリソースにアクセスするための新しい OAuth クレデンシャルを作成します。
...
* 必要な Toolkit モジュール  
  このチュートリアルでは、コンポーザブル [`MapView`](https://developers.arcgis.com/kotlin/toolkit-api-reference/arcgis-maps-kotlin-toolkit/com.arcgismaps.toolkit.geoviewcompose/-map-view.html) を含む `geoview-compose` モジュールと `authentication` モジュールが必要です。
...
#### ユーザー認証
  ...
  2. `clientID` と `redirectURL` の値を設定します。[認証設定](#認証の設定) ステップの`ユーザー認証`パートでアプリに提供した **RedirectURL** を使用する必要があります。
