+++
title = "レイヤー"
description = "レイヤーは、ArcGIS を使用して構築するアプリケーションの主要なコンテンツです。ArcGIS で利用できるレイヤーは、ディスク上またはネットワーク経由で利用可能な ArcGIS REST API 、CSV ファイル、GeoJSON、また他のファイルを介して公開される Web サービスに基づいています。レイヤーの種類と特長について紹介します。"
Weight=4
aliases = ["/layers/"]
+++
