+++
title = "Web マップ"
description = "Web マップは、異なるアプリケーション間で作成、スタイル設定、共有できる 2D マップです。Web マップは Web マップ仕様に基づいた JSON で構成されており、マップの範囲、ベースマップ、レイヤ、レイヤ スタイル、ポップアップなどの設定が含まれています。Web マップの仕様や作成、表示方法についてご紹介します。"
Weight=5
aliases = ["/web-maps/"]
+++

## Web マップとは？
Web マップは、異なるアプリケーション間で作成、スタイル設定、共有できる 2D マップです。Web マップは Web マップ仕様に基づいた JSON で構成されており、マップの範囲、ベースマップ、レイヤ、レイヤ スタイル、ポップアップなどの設定が含まれています。
...
## なぜ Web マップを使うのか？
Web マップは、[ArcGIS Online](https://developers.arcgis.com/glossary/arcgis-online/) および [ArcGIS Pro](https://developers.arcgis.com/glossary/arcgis-pro/) で作成し、[Navigator](https://www.esri.com/en-us/arcgis/products/navigator-for-arcgis/resources)、[Collector](https://www.esri.com/en-us/arcgis/products/collector-for-arcgis/overview)、[ArcGIS StoryMaps](https://storymaps.arcgis.com/)、[ArcGIS Configurable Apps](https://www.esri.com/en-us/arcgis/products/configurable-apps/overview) や [ArcGIS API および SDK](https://developers.arcgis.com/documentation/#sdks) を使用して構築されたアプリなど、多くのアプリケーションで表示することができます。アプリケーションが Web マップをロードすると、以前に保存した設定が自動的にロードされるため、アプリケーションの開発や [ArcGIS プラットフォーム](https://developers.arcgis.com/glossary/arcgis-platform/)全体での 2D マップの共有が容易になります。
...
JSON として、Web マップは ArcGIS Server 上でホストされ、REST API を介して使用することができます。技術的な側面でに言うと、Web マップは、ArcGIS Online または ArcGIS Enterprise に保存された[コンテンツ アイテム](https://developers.arcgis.com/rest/users-groups-and-items/item.htm)のことを指します。
...
[ArcGIS Online のドキュメント](https://doc.arcgis.com/en/arcgis-online/get-started/get-started-with-maps.htm)や ArcGIS チュートリアルを参照して、[Web マップを作成](https://developers.arcgis.com/labs/arcgisonline/create-a-web-map/index.html)することができます。

## ArcGIS Pro
ArcGIS Pro では、Web マップの作成と共有がサポートされています。ArcGIS Pro ヘルプの [Web マップの作成](http://pro.arcgis.com/en/pro-app/help/mapping/map-authoring/author-a-web-map.htm)を参照してください。
...
* Web マップの作成 - [Android](https://developers.arcgis.com/android/latest/guide/build-a-new-map.htm),[iOS](https://developers.arcgis.com/ios/latest/swift/guide/build-a-new-map.htm), [Java](https://developers.arcgis.com/java/latest/guide/build-a-new-map.htm), [.NET](https://developers.arcgis.com/net/latest/wpf/guide/build-a-new-map.htm), [Qt](https://developers.arcgis.com/qt/latest/qml/guide/build-a-new-map.htm)
...
## ArcGIS API for JavaScript
ArcGIS API for JavaScript を使用して Web マップを表示する方法については、[Web マップを表示するチュートリアル](https://developers.arcgis.com/labs/javascript/display-a-web-map/index.html)を参照してください。

## ArcGIS Runtime SDK
すべての ArcGIS Runtime SDK には、Web マップをレンダリングする機能があります。Web マップを表示する方法については、[ArcGIS チュートリアル](https://developers.arcgis.com/labs/)を参照してください。

[Android](https://developers.arcgis.com/labs/android/display-a-web-map/index.html), [iOS](https://developers.arcgis.com/labs/ios/display-a-web-map/index.html), [Java](https://developers.arcgis.com/labs/java/display-a-web-map/index.html), [.NET](https://developers.arcgis.com/labs/net/display-a-web-map/index.html),  [Qt](https://developers.arcgis.com/labs/qt/display-a-web-map/index.html) で Web マップを表示する。

## ArcGIS Configurable apps
ArcGIS Configurable apps を使用した Web マップを ArcGIS Onlineで 表示することもできます。

アプリケーションを作成するには、[ArcGIS Online でマップを作成するチュートリアル](https://learn.arcgis.com/en/projects/create-a-map/)を参照してください。

または、[ArcGIS Configurable apps を使用したサンプル](https://doc.arcgis.com/en/arcgis-online/create-maps/choose-configurable-app.htm)を参照してください。
